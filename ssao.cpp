...
class Sample : public nvgl::AppWindowProfilerGL
{
  ...
  struct
  {
    nvgl::ProgramID draw_scene, depth_linearize, depth_linearize_msaa, viewnormal, bilateralblur, displaytex,

        hbao_calc, hbao_calc_blur, hbao_blur, hbao_blur2,

        hbao2_deinterleave, hbao2_calc, hbao2_calc_blur, hbao2_reinterleave, hbao2_reinterleave_blur;
...
void Sample::processUI(double time)
{
  ...
  ImGui::NewFrame();
  ImGui::SetNextWindowSize(ImGuiH::dpiScaled(320, 0), ImGuiCond_FirstUseEver);
...
void Sample::drawLinearDepth(const Projection& projection, int width, int height, int sampleIdx)
{
  ...
  if(m_tweak.samples > 1)
  {
    ...
    glUniform4f(0, projection.nearplane * projection.farplane, projection.nearplane - projection.farplane,
...
void Sample::drawHbaoCacheAware(const Projection& projection, int width, int height, int sampleIdx)
{
...

  {
    ...
    {
      glUniform4f(0, float(i % 4) + 0.5f, float(i / 4) + 0.5f, m_hbaoUbo.InvFullResolution.x,
                  m_hbaoUbo.InvFullResolution.y);
